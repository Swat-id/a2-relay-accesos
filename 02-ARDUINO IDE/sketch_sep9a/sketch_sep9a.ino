...
void processKey(uint8_t key, int keyboardId) {
 ...
 void printSystemStatus(unsigned long currentTime) {
  ...
  Serial.printf("   📍 TECLADO 1 (GPIO%d/%d): PIN='%s' (%d chars)\n", 
                WIEGAND1_D0, WIEGAND1_D1, currentPin1, strlen(currentPin1));
  Serial.printf("   📍 TECLADO 2 (GPIO%d/%d): PIN='%s' (%d chars)\n", 
                WIEGAND2_D0, WIEGAND2_D1, currentPin2, strlen(currentPin2));
  ...
  if (strlen(lastCode) > 0) {
    Serial.printf("📋 Último acceso: %s %s a las %s (Teclado %d)\n", 
                  lastType, lastCode, lastTime, lastKeyboardId);
  }
...
void handleBulkImport() {
  ...
  // Generar respuesta
  String response = "<html><body><h1>📥 Resultado de Importación Masiva</h1>";
  response += "<p><strong>Códigos importados:</strong> " + String(importedCount) + "</p>";
  response += "<p><strong>Errores:</strong> " + String(errorCount) + "</p>";
...
void handleCodes() {
 ...
 <body>
  ...
  if (displayedCount == 0) {
   ...
   <!-- NUEVA SECCIÓN: IMPORTACIÓN MASIVA DE CÓDIGOS -->
   <div style='background: #fff; padding: 20px; border-radius: 10px; margin: 20px 0; box-shadow: 0 2px 8px rgba(0,0,0,0.1);'>
     <h3><i class='fas fa-upload'></i> Importación Masiva de Códigos</h3>
     <form action='/codes/bulk-import' method='post' enctype='multipart/form-data'>
       <label><strong>Archivo CSV:</strong></label><br>
       <input type='file' name='csvFile' accept='.csv' required style='margin: 10px 0;'><br>
       <button type='submit' style='background-color: #27ae60; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer;'>
         <i class='fas fa-upload'></i> Importar Códigos
       </button>
     </form>
     <p><small><strong>Formato CSV:</strong> Tipo,Codigo,Teclado,Rele,Fecha_Creacion<br>
     <strong>Tipos soportados:</strong> PIN (4-6 dígitos), TAG (1-16 caracteres)<br>
     <strong>Nota:</strong> Todos los campos son obligatorios en el CSV</small></p>
   </div>
   
   <!-- NUEVA SECCIÓN: LECTURA DE TAGS EN TIEMPO REAL -->
   <div style='background: #fff; padding: 20px; border-radius: 10px; margin: 20px 0; box-shadow: 0 2px 8px rgba(0,0,0,0.1);'>
