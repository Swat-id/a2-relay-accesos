/*
 * KC868-A2 Dual Wiegand Security Controller
 ...
 */
 ...
 const char* firmwareVersion = "1.7.0-COMPLETE-SECURITY";
 
 // =================== IDENTIFICACIÓN DEL DISPOSITIVO ===================
 String fixedSerialNumber;  // SERIAL FIJO PARA MQTT - NO MODIFICABLE
 String deviceName = "";    // NOMBRE EDITABLE POR USUARIO
 bool useDhcp = true;
...
#define MAX_CODES 500  // Aumentado a 500 códigos
...
String getTimeString() {
 ...
 void processKey(uint8_t key, int keyboardId) {
  ...
  if (keyChar == '#') {
     ...
     *currentPin = "";  // Limpiar PIN siempre después de #
     ...
     if (currentPin->length() < maxPinLength) {
       *currentPin += keyChar;
       Serial.printf("📝 [TECLADO %d] PIN parcial: %s (longitud: %d/%d)\n", 
                     keyboardId, currentPin->c_str(), currentPin->length(), maxPinLength);
     } else {
 ...
 void printSystemStatus(unsigned long currentTime) {
  ...
  if (mqttClient.connected()) {
    ...
    Serial.printf(" (NO REQUERIDO - MODO AUTÓNOMO)");
  }
  Serial.println();
   ...
   if (localAccessBlocked && blockStartTime > 0) {
     unsigned long remaining = (blockDuration > (millis() - blockStartTime)) ? 
       (blockDuration - (millis() - blockStartTime)) / 1000 : 0;
     Serial.printf("   ⏰ Tiempo restante bloqueo: %lu segundos\n", remaining);
   }
   ...
   // Estado de relés - LÍNEA CORREGIDA
   Serial.printf("⚡ Relés:\n");
   for (int i = 1; i <= 2; i++) {
     Serial.printf("   R%d: %s", i, releActive[i] ? "🟢 ON" : "🔴 OFF");
     if (releActive[i]) {
       // CORRECCIÓN: Usar operador ternario en lugar de max() para evitar conflicto de tipos
   ...
   // Último acceso
   if (lastCode != "") {
     Serial.printf("📋 Último acceso: %s %s a las %s (Teclado %d)\n", 
                   lastType.c_str(), lastCode.c_str(), lastTime.c_str(), lastKeyboardId);
   }
 ...
 void updateConfiguration(const JsonDocument& doc) {
   if (doc.containsKey("message_info")) {
     if (doc["message_info"].containsKey("device_name")) {
       deviceName = doc["message_info"]["device_name"].as<String>();
     }
 ...
 void loadConfiguration() {
   ...
   if (config.configValid == 0xABCD1234) {
     ...
     deviceName = fixedSerialNumber; // Nombre inicial igual al serial
     useDhcp = true;
     releDuration = 2.0;
     localAccessBlocked = false;
     failedAttempts = 0;
...
void loadTurnstileConfig() {
  ...
  if (!config.turnstile.enabled) {
    ...
    Serial.println("🔄 Configuración del modo torno inicializada por defecto");
  } else {
    Serial.println("🔄 Configuración del modo torno cargada desde EEPROM");
...
void checkPendingRequestTimeout() {
  ...
  if (elapsedTime > TURNSTILE_TIMEOUT) {
    ...
    Serial.printf("⏰ [TORNO] [%s] Timeout de solicitud (%lu ms) - Abriendo Relé %d automáticamente\n", 
                  keyboardName.c_str(), elapsedTime, pendingRequest.relay_to_open);
    
    // Abrir relé según teclado origen (timeout = aprobación automática)
    controlReleWithDuration(releDuration, pendingRequest.relay_to_open);
    ...
    // Publicar evento de acceso por timeout
    publishTurnstileEvent(String(pendingRequest.code), String(pendingRequest.type), pendingRequest.keyboard_id, true, "TORNO_TIMEOUT");
...
void handleImportCodes() {
  ...
  // Generar respuesta
  String response = "<html><body><h1>📥 Resultado de Importación</h1>";
...
void loadStoredCodes() {
  ...
  if (storedCodes.validMarker != 0xCAFEBABE) {
    ...
    Serial.printf("💾 Códigos cargados: %d (versión %d)\n", storedCodes.count, storedCodes.version);
  }
}
 ...
 void saveStoredCodes() {
   ...
   EEPROM.commit();
   Serial.println("💾 Códigos guardados en EEPROM");
 }
...
bool isCodeStored(const char* type, const char* value, int* relay) {
 ...
 void handleRoot() {
   ...
   html += "<title>KC868-A2 Dual Wiegand Controller</title>";
   ...
   html += "<header><h1><i class='fas fa-keyboard icon'></i>KC868-A2 Dual Wiegand Controller</h1>";
   html += "<p>Device: <strong>" + deviceName + "</strong> | Serial: <strong>" + fixedSerialNumber + "</strong></p>";
   html += "<p>IP: " + ip.toString() + " | Firmware: <strong>" + firmwareVersion + "</strong></p></header>";
  ...
  html += "<tr><td>Modo torno</td><td>" + String(isTurnstileModeEnabled() ? "🟢 ACTIVO" : "🔴 INACTIVO") + "</td></tr>";
   ...
   html += "</div></main>";
   html += "<script>function toggleIpFields(){document.getElementById('staticIpFields').style.display=(document.getElementById('useDhcp').value=='1')?'none':'block';}</script>";
   html += "</body></html>";
 ...
 <body>
   <h1><i class='fas fa-keyboard'></i> Gestión de Códigos - Sistema Dual Wiegand</h1>
  ...
  <div class='export-section'>
    <a href='/export/codes'><button class='btn-info'><i class='fas fa-download icon'></i>Exportar a CSV</button></a>
    <form action='/import/codes' method='post' enctype='multipart/form-data' style='display:inline;'>
      <input type='file' name='csvFile' accept='.csv' required style='margin:0 10px;'>
      <button type='submit' class='btn-info'><i class='fas fa-upload icon'></i>Importar CSV</button>
    </form>
  </div>
 ...
 void setup() {
   ...
   Serial.println("║                  Firmware v1.7.0-COMPLETE                  ║");
   Serial.println("╚══════════════════════════════════════════════════════════════╝");
  ...
  timeSynced = false;
  currentTimeString = "No sincronizado";
 ...
 void loop() {
  ...
  checkPendingRequestTimeout();
   ...
   if (isTurnstileModeEnabled() && pendingRequest.active) {
     if (currentPin1.length() > 0 && (currentTime - lastKeyPressTime1 > keyPressTimeout)) {
       Serial.printf("⏰ [TECLADO 1] Timeout PIN en modo torno (%lu ms) - Limpiando: '%s' (solicitud pendiente activa)\n", 
                     keyPressTimeout, currentPin1.c_str());
       currentPin1 = "";
     }
     
     if (currentPin2.length() > 0 && (currentTime - lastKeyPressTime2 > keyPressTimeout)) {
       Serial.printf("⏰ [TECLADO 2] Timeout PIN en modo torno (%lu ms) - Limpiando: '%s' (solicitud pendiente activa)\n", 
                     keyPressTimeout, currentPin2.c_str());
       currentPin2 = "";
     }
   }
...
<body>
  <h1><i class='fas fa-cloud'></i> Gestión de Códigos Remotos - Sistema Dual Wiegand</h1>
...
void handleRemoteCodesAdd() {
  ...
  if (server.hasArg("type") && server.hasArg("value") && server.hasArg("relay") && server.hasArg("keyboard_id")) {
    ...
    server.send(200, "text/html",
      "<html><body><h1>❌ Error al añadir código remoto</h1>"
      "<p>Verifique el formato y que no exista ya</p>"
